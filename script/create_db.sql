...
CREATE TABLE IF NOT EXISTS `grp24`.`inventory` (
  ...
  INDEX `fk_inventory_brand1_idx` (`brand_brand_id` ASC),
...
CREATE TABLE IF NOT EXISTS `grp24`.`address` (
  ...
  INDEX `fk_address_street1_idx` (`street_street_id` ASC),
...
CREATE TABLE IF NOT EXISTS `grp24`.`item` (
  ...
  INDEX `fk_item_inventory1_idx` (`inventory_inventory_id` ASC),
  ...
  CONSTRAINT `fk_item_item_subtype1`
    ...
    ON DELETE RESTRICT
    ON UPDATE CASCADE,
  CONSTRAINT `fk_item_inventory1`
    FOREIGN KEY (`inventory_inventory_id`)
    REFERENCES `grp24`.`inventory` (`inventory_id`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE)
ENGINE = MyISAM;
